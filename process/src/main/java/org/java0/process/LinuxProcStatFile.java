    ...
    public LinuxProcessInfo getInfo() {
        ...
        if (!invalidPid) {
            try {
                ...
                linuxProcessInfo = new LinuxProcessInfo(line);
            } catch (final ParseException e) {
                logger.error("Parse exception", e);
                logger.error("Unable to parse proc/stat line: {}", line);
            } catch (final FileNotFoundException e) {
                logger.debug("Process {} went away", pid);
                invalidPid = true;
            } catch (final IOException e) {
                /*
                 * We get here with an "java.io.IOException: No such process" if
                 * we tried to read the process file for a process that no
                 * longer exists. Unfortunately, we can't easily tell if the
                 * IOException was the result of something more unusual. For
                 * now, we'll just treat is as a normal process disappearance
                 */
                logger.debug("Process {} went away", pid);
                invalidPid = true;
            }
