...
/**
 * Returns singleton cached by this class. Otherwise, tries to find the object
 * in the root factory. If not found, creates the object using the no-arg
 * constructor.
 * 
 * @author Hugh Eaves
 *
...
public class DefaultProvider<T> extends AbstractTypedObjectProvider<T> {
    private static final Logger logger = Logger.getLogger(DefaultProvider.class.getName());
    ...
    public DefaultProvider(final Class<? extends T> type, final boolean singleton) {
        this(type, null, singleton);
    }

    public DefaultProvider(final Class<? extends T> type, final Tag tag) {
        this(type, null, false);
    }

    public DefaultProvider(final Class<? extends T> type, final Tag tag, final boolean singleton) {
        super(type);
        this.tag = tag;
