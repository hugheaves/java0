...
public class CLIUtil {
	...
	public static int processCommandLine(final String[] args,
	...
	private static Command parseCommandLine(final String[] args,
			final Command... commands) {
		...
		try {
			programName = commands[0].getClass().getProtectionDomain()
		...
		try {
			...
			if (commands.length == 1) {
				...
				final Parameters parameters = command.getClass().getAnnotation(
						Parameters.class);
			...
			if (!singleDefault) {
				...
				if (commandName != null) {
					...
					throw new ParameterException(
							"No command given on commandline");
				}
			}
